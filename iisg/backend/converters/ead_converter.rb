...
class EADConverter < Converter
  ...
  def self.configure
    ...
    with 'unitid' do |node|
      ancestor(:note_multipart, :resource, :archival_object) do |obj|
        ...
        when 'resource'
          if inner_xml =~ /^(ARCH|COLL)[\/_\-\.\s]?[0-9]*$/
            set obj, :id_0, inner_xml[0..3] if obj.id_0.nil? || obj.id_0.empty?
            set obj, :id_1, inner_xml.scan(/\d/).join if obj.id_1.nil? || obj.id_1.empty?
          else
    ...
    %w(accessrestrict accessrestrict/legalstatus
      ...
      with note do |node|
        ...
        if att('type') && node.name === 'accessrestrict'
          a_hash.store(:rights_restriction, { :local_access_restriction_type => [att('type')] })
        end

        make :note_multipart, a_hash do |note|
          set ancestor(:resource, :archival_object), :notes, note
        end
      end
    end
    ...
    with 'daogrp' do |*|
      ...
      make :digital_object, {
        ...
        dg_contents.xpath('daoloc').each do |daoloc|
          ...
          fv_attrs[:label] = daoloc['xlink:label'] if daoloc['xlink:label']
  ...
  def make_corp_template(opts)
    ...
    subarea = corpname.xpath('subarea')
    corpname.xpath('subarea').remove if subarea
    primary_name = normalize(corpname.inner_text)
    ...
    a_hash.store(:subordinate_name_1, subarea.inner_text) if subarea

    make :name_corporate_entity, a_hash do |name|
  ...
  def make_person_template(opts)
    ...
    primary_name, rest_of_name = inner_xml.strip.split(',', 2)
    a_hash = {
      :name_order => 'inverted',
      :primary_name => primary_name,
      :authority_id => att('authfilenumber'),
    ...
    a_hash.store(:rest_of_name, rest_of_name) if rest_of_name

    make :name_person, a_hash do |name|
