...
class EADSerializer < ASpaceExport::Serializer
        ...
        xml.archdesc(atts) {
          ...
          xml.did {
            ...
            if (val = data.title)
              ...
              xml.unittitle({:label=>'Title'})  {   sanitize_mixed_content(val, xml, @fragments) }
            end
            ...
            if (val = data.language)
                    ...
                    xml.langmaterial({:label => 'Language of Material'}) {

                    # modified
                    #xml.language(:langcode => val) {
                    #xml.language({:langcode => val, :encodinganalog => '041$a', :scriptcode => 'Latn'}) {
                    #xml.language({:langcode => val, :scriptcode => 'Latn'}) {
                    xml.language({:langcode => val}) {
            ...
            if (val = data.repo.name)
                ...
                xml.repository ({:label=>'Repository'}) {
            ...
            # TODO: WAT IS DIT? MET UNDERSCORE ?
  ...
  def extract_p_text(content, backup = "")
    content ||= ""
    match = content.strip.match(/<p( [^<>]+)?>(.+?)<\/p>/)
    if match.nil? # content has no head so we return it as it
      return [content, backup ]
    else
      [ content.gsub(match.to_a.first, ''), match.to_a.last]
  ...
  def serialize_child(data, xml, fragments, c_depth = 1)
    ...
    xml.send(tag_name, atts) {
      ...
      serialize_nondid_notes(data, xml, fragments, false)
  ...
  def serialize_digital_object(digital_object, xml, fragments)
    ...
    if file_versions_to_display.empty?
      ...
      href = file_version['file_uri']

      # speciaal geval voor label=handle.
      if href.include? '/10622/'
      ...
      xml.daogrp( atts.merge( { 'xlink:type' => 'extended', 'xlink:title' => 'Digital Object' } ) ) {
        ...
        file_versions_to_display.each do |file_version|
          href = file_version['file_uri']
          if href.include? '/10622/' # speciaal geval voor label=handle.
            explode(xml, href)
          else
...
#					}
  ...
  def serialize_note_content(note, xml, fragments, use_head = true)
    ...
    atts = {:id => prefix_id(note['persistent_id']) }.reject{|k,v| v.nil? || v.empty? || v == "null" }.merge(audatt)
      ...
      if note['rights_restriction']
        if note['rights_restriction']['local_access_restriction_type']
              separator = ''
              note['rights_restriction']['local_access_restriction_type'].each do |subnote|
                part_text = part_text + separator + subnote
                separator = ', '
              end
        end
      end
      atts[:type] = part_text # restriction
  ...
  def serialize_nondid_notes(data, xml, fragments, use_head = true)
    data.notes.each do |note|
      next if note["publish"] === false && !@include_unpublished
      ...
      if note['type'] == 'legalstatus'
        xml.accessrestrict(audatt) {
          serialize_note_content(note, xml, fragments, use_head)
        }
      else
        serialize_note_content(note, xml, fragments, use_head)
      end
  ...
  def serialize_eadheader(data, xml, fragments)
    ...
    xml.eadheader(eadheader_atts) {
      ...
      eadid_identifier = data.ead_id.sub('http://hdl.handle.net/', '').sub('https://hdl.handle.net/', '').sub('hdl:', '') # 12345/abcdefg
      eadid_atts = {:countrycode => data.repo.country,
              :url => eadid_url,
              :identifier => 'hdl:' + eadid_identifier,
              :mainagencycode => data.mainagencycode}.reject{|k,v| v.nil? || v.empty? || v == "null" }

      xml.eadid(eadid_atts) {
        xml.text 'https://hdl.handle.net/' + eadid_identifier
      }
  ...
  def explode(xml, href)
    a={'xlink:type'=>'locator'}
    { 'manifest' => 'manifest', 'catalog' => 'catalog', 'level3' => 'thumbnail' }.each do |k, v|
      a['xlink:href'] = href + '?locatt=view:' + k
      a['xlink:label'] = v
      xml.daoloc(a)
    end
  end
