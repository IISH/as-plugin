...
#        xml.controlfield(:tag => '000') {
#          xml.cdata marc.controlfield_string_000
#        }
        xml.controlfield(:tag => '001') {
          xml.text marc.controlfield_string_001
        }
        xml.controlfield(:tag => '003') {
          xml.text marc.controlfield_string_003
        }
        xml.controlfield(:tag => '008') {
          xml.text marc.controlfield_string_008
        }
        sorted_datafields = marc.datafields.sort {|a, b| a.tag <=> b.tag}

        sorted_datafields.each do |df|

          if df.subfields.size > 0
