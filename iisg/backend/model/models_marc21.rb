class MARCModel < ASpaceExport::ExportModel
  ...
  @archival_object_map = {
    [:id_0, :id_1, :id_2, :id_3] => :resource_identifier,
  ...
  attr_accessor :controlfield_string_000
  attr_accessor :controlfield_string_001
  attr_accessor :controlfield_string_003
  attr_accessor :controlfield_string_008
  ...
  def self.from_resource(obj, opts = {})
    ...
    marc.controlfield_string_000 = obj
    marc.controlfield_string_003 = main_agency_code(obj) # dit komt uit een source element. Moet zijn:
    marc.controlfield_string_008 = assemble_controlfield_string_008(obj)

    marc
  end

  def self.assemble_controlfield_string_008(obj)
    date = obj.dates[0] || {}
    ...
    geocode = nil
    obj['subjects'].each do |link|
      subject = link['_resolved']
      term, *terms = subject['terms']
      if term['term_type'] == 'geographic'
        geoname = term['term']
        geocode = Enumeration[:name => 'country_iso_3166'][geoname]
        break if geocode
      end
    end
    ...
    # If only one Language and Script subrecord its code value should be exported in the MARC 008 field position 35-37;
    # If more than one Language and Script subrecord is recorded, a value of "mul" should be exported in the MARC 008 field position 35-37.
    # Maar kennelijk niet bij het iisg
    lang_materials = obj.lang_materials
    languages = lang_materials.map { |l| l['language_and_script'] }.compact
    langcode = languages.count == 0 ? '   ' : languages[0]['language']
  ...
  def self.main_agency_code(obj)
    repo = obj['repository']['_resolved']
    @mainagencycode=''
    if repo.has_key?('country') && !repo['country'].empty? && repo.has_key?('org_code') && !repo['org_code'].empty?
      @mainagencycode = [repo['country'].upcase, repo['org_code']].join('-')
    else
      @mainagencycode='NL-AmISG'
    end
    @mainagencycode
  end
  ...
  def resource_identifier(*ids)
    ids.reject! {|i| i.nil? || i.empty?}
    @id = ids.join('')
    identifier = '10622' + '/' + @id
    @controlfield_string_001 = identifier

    df('856', '4', ' ').with_sfs(['q', 'text/xml'], ['u', 'https://hdl.handle.net/' + identifier + '?' + 'locatt=view:ead'])
    df('902', ' ', ' ').with_sfs(['a', identifier])
  end
